using BeforeOurTime.Gossip.Messaging;
...
namespace BeforeOurTime.Gossip
{
    public class GossipClient
    {
        ...
        {
            var authenticateBlock = new Authentication(ConfigSettings);
        ...
        private void OnMessage(object sender, MessageEventArgs e)
        {
            ...
            switch (newReply.Event)
            {
                ...
                case "tells/receive":
                    ...
                    var validPlayer = UserList().FirstOrDefault(user => user.Name.Equals(myName, StringComparison.InvariantCultureIgnoreCase));
                ...
                case "channels/broadcast":
                    ...
                    {
                        foreach (var user in UserList().Where(usr => !usr.BlockedMembers.Contains(messageSender)))
                ...
                case "players/sign-in":
                    if (newReply.ReferenceID == null)
                    {
                        ...
                        foreach (var user in UserList().Where(usr => usr.Friends.Contains(fullSignInName)))
                ...
                case "players/sign-out":
                    if (newReply.ReferenceID == null)
                    {
                        ...
                        foreach (var user in UserList().Where(usr => usr.Friends.Contains(fullSignoutName)))
